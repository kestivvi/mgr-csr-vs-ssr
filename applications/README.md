# Research Applications Specification (MGR Project)
...
Each directory herein is a **standardized scientific instrument** (an **Application**) designed to measure the resource cost of specific rendering strategies under identical conditions.
...
- **Web Server / Proxy**: Every application container must include a front-facing web server (Nginx/Apache) acting as a reverse proxy.
...
- **Global Styles**: All applications must utilize the identical global CSS file provided in the repository.
- **Standardized Titles**: The `<title>` tag must follow the pattern: `MGR - [Application ID]`.
...
The project currently encompasses **31 Research Applications** (10 CSR/SSG, 21 SSR). The registry is maintained automatically via **Application Autodiscovery**.

### SSR (Server-Side Rendering)
| Application ID | Framework | Runtime |
| :--- | :--- | :--- |
...
| Application ID | Framework / Type | Web Server |
| :--- | :--- | :--- |
...
./venv/bin/mgr verify --apps [application-id]
```
