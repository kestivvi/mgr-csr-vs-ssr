...
def mock_artifact(tmp_path: Path) -> ResearchArtifact:
    ...
    metadata = {
        ...
        "subjects": {"mock-app": subject_data}
    }
...
def test_experiment_loader_captures_structured_subject_metadata(mock_artifact: ResearchArtifact):
    loader = ExperimentLoader()
    experiment = loader.load(mock_artifact)
    
    assert "subjects" in experiment.metadata
    assert experiment.metadata["subjects"]["mock-app"]["family"] == "solid"
    assert experiment.metadata["subjects"]["mock-app"]["meta_framework"] == "solid-start"
    assert experiment.metadata["subjects"]["mock-app"]["strategy"] == "ssr"
    assert experiment.metadata["subjects"]["mock-app"]["runtime"] == "bun"

def test_experiment_loader_legacy_fallback_fails_fast(tmp_path: Path):
