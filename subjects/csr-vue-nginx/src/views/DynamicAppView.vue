...
function generate(id: string | string[] | undefined, n: number): Row[] {
  const idStr = Array.isArray(id) ? id[0] : id;
  const seed = parseInt(idStr ?? '', 10) || 1;
...
const rows = computed(() => generate(route.params.id, ROW_COUNT));
const summary = computed(() => rows.value.reduce(
  (acc, r) => ({
    count: acc.count + 1,
...
.dynamic-app :deep(.badge) { display: inline-block; padding: 0.1rem 0.45rem;
  border-radius: 3px; font-size: 0.7rem; font-weight: 700;
  min-width: 2rem; text-align: center; }
.dynamic-app :deep(.badge-in) { background: #d4edda; color: #155724; }
.dynamic-app :deep(.badge-out) { background: #f8d7da; color: #721c24; }
</style>
